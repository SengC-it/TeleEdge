...
begin
  ...
  insert into public.teleeg_outbox (
    ...
    'entry', v_candidate.signal_id,
    '[Tele-Signal] ' || upper(v_candidate.side) || ' ' || v_candidate.market_id,
    format(E'Strategy: %s / %s\nEntry: %s\nSL: %s\nTP: %s (%sR)\nQuantity: %s\nPlanned risk: %s USDT\nSignal candle close: %s\nPaper only; TP/SL first touch settles; same 1m candle uses SL; no time exit.',
      v_candidate.family, v_candidate.edge_segment, v_candidate.entry,
      v_candidate.stop, v_candidate.target, v_candidate.target_r,
...
begin
  ...
  insert into public.teleeg_outbox (
    ...
    'exit:' || p_signal_id,
    'exit', p_signal_id,
    '[Tele-Alerts] ' || upper(p_exit_reason) || ' ' || v_position.market_id,
    format(E'Direction: %s\nEntry / exit: %s / %s\nGross PnL: %s USDT\nFunding: %s USDT\nModeled trading cost: %s USDT\nNet PnL: %s USDT\nSettlement time: %s',
      upper(v_position.side), v_position.entry, p_exit_price,
