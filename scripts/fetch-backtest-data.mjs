...
async function fetchPaged(pathname, parameters, {rowLimit = 1500, mapRow}) {
  const rows = [];
...
function writeArtifact(directory, symbol, rows) {
  fs.mkdirSync(directory, {recursive: true});
  const file = path.join(directory, `${symbol}.json.gz`);
  fs.writeFileSync(file, zlib.gzipSync(JSON.stringify(rows)));
  return {path: path.relative(APP_DIR, file).replaceAll('\\', '/'), rows: rows.length, sha256: sha256(file)};
}
