...
The local service records each strategy gate directly. The Edge worker records shard-level history, candidate-family gate passes, ranking, acceptance, and data errors; the finalize job merges all successful shard summaries before publishing `last_scan_summary` to the dashboard. A missing candidate is reported as `no_candidate` rather than silently becoming a zero-candidate scan.
